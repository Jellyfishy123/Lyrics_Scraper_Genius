...
TKN = 'UCXBKlJRmgBndcj6g4nfqOOOao034djqVU4KZd3USwKSfq7s1QSp5e9yo2ugLbJ2' # get it on https://genius.com/api-clients by creating a client and clicking Generate Access Token
...
def scrape(link):
    ...
    filename = "Khanh Ly.txt"

    with open(filename, 'w', encoding='utf-8') as file:
        file.write(''.join(lyricsform))
...
def search(query_term):
    ...
    for lyricsdata in BeautifulSoup(get(urls_list[int(link_num)]).text, 'html.parser').select('div[class*=Lyrics__Container]'):
        dat = lyricsdata.get_text('\n')
        lyricsform.append(f"{dat}\n")
    ...
    filename = os.path.join("C:/Users/tiend/Spotify-Lyrics/pylyrical/Diem Quyen", f"{search_results[int(link_num)]}.txt".replace("&", "and").replace("/", "and").replace("?", ""))
